...
class interfaz_Brazo(tk.Frame):
    ...
    def _init_axes(self):
        ...
        self.ax1.set_title('Posición del Brazo ')
        self.ax1.set_xlabel("x")
        self.ax1.set_ylabel("y")
        self.ax1.set_zlabel("z")
        self.ax1.set_xlim(-20, 20)
        self.ax1.set_ylim(-20, 20)
        self.ax1.set_zlim(-1, 20)

    
    #Boton guardar grafica
    def guardar(self):
        if self.btn_guardar["text"] == "Guardar Figura":
...
if __name__ == "__main__":
    root = tk.Tk()
    interfaz_Brazo(root).pack(side="top", fill="both", expand=True)
    root.mainloop()
